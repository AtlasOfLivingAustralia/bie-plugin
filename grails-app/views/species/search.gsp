...
<section class="${fluidLayout ? 'container-fluid' : 'container'}">
    ...
    <header class="pg-header">
        <div class="row">
            <div class="col-sm-9">
                <h1>
                    <g:message code="search.for"/>
                    <strong>${searchResults.queryTitle == "*:*" ? 'everything' : searchResults.queryTitle}</strong>
                    <g:message code="search.returned"/>
                    <g:formatNumber number="${searchResults.totalRecords}" type="number"/>
                    <g:message code="search.results"/>.
                </h1>
            </div>
            ...
            <div class="row">
                <div class="col-sm-3">
                    <div class="well refine-box">
                        ...
                        <h2 class="visible-xs">
                            <a href="#refine-options" data-toggle="collapse"><span
                                    class="glyphicon glyphicon-chevron-down" aria-hidden="true"></span>
                                <g:message code="search.resultFine"/>
                            </a>
                        </h2>
                                            ...
                                            <li>
                                                ...
                                                <g:else>
                                                    <g:message code="facet.${item.key}"
                                                               default="${item.key}"/>: <strong><g:message
                                                        code="${item.key}.${item.value}"
                                                        default="${item.value}"/></strong>
                                                </g:else>
                                    ...
                                    <ul class="list-unstyled">
                                        ...
                                        <g:each var="fieldResult" in="${facetResult.fieldResult}" status="vs">
                                            ...
                                            <g:if test="${facetResult.fieldName?.contains("occurrence_date") && fieldResult.label?.endsWith("Z")}">
                                                <li><g:set var="startYear"
                                                           value="${fieldResult.label?.substring(0, 4)}"/>
                                            ...
                                            <g:elseif
                                                    test="${fieldResult.label?.endsWith("before")}"><%-- skip --%></g:elseif>
                        ...
                        <form class="form-inline">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control input-sm" id="sort-by" name="sort-by">
                                    <option value="score" ${(params.sortField == 'score') ? "selected=\"selected\"" : ""}><g:message
                                            code="search.sort.match"/></option>
                                    <option value="scientificName" ${(params.sortField == 'scientificName') ? "selected=\"selected\"" : ""}><g:message
                                            code="search.sort.scientific"/></option>
                                    <option value="commonNameSingle" ${(params.sortField == 'commonNameSingle') ? "selected=\"selected\"" : ""}><g:message
                                            code="search.sort.common"/></option>
                                    <option value="rank" ${(params.sortField == 'rank') ? "selected=\"selected\"" : ""}><g:message
                                            code="search.sort.taxon"/></option>
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control input-sm" id="sort-order" name="sort-order">
                                    <option value="asc" ${(params.dir == 'asc') ? "selected=\"selected\"" : ""}><g:message
                                            code="search.sort.order.ascending"/></option>
                                    <option value="desc" ${(params.dir == 'desc' || !params.dir) ? "selected=\"selected\"" : ""}><g:message
                                            code="search.sort.order.desceding"/></option>
                                </select>
                            </div>
                        </form>
                        ...
                        <g:each var="result" in="${searchResults.results}">
                            <li class="search-result clearfix">
                                ...
                                <g:if test="${result.has("idxtype") && result.idxtype == 'TAXON'}">
                                    ...
                                    <h3>${result.rank}:
                                        <a href="${acceptedPageLink}">
                                            <bie:formatSciName rankId="${result.rankID}"
                                    ...
                                    --%><g:if test="${result.favourite}"><span
                                            class="favourite favourite-${result.favourite}"
                                            title="<g:message code="favourite.${result.favourite}.detail"/>"><g:message
                                                code="favourite.${result.favourite}" encodeAs="raw"/></span></g:if>
                                    </h3>
                                    ...
                                    <g:each var="fieldToDisplay"
                                        ...
                                        <g:if test='${result."${fieldToDisplay}"}'>
                                            <p class="summary-info"><strong>
                                                <g:message code="${fieldToDisplay}"
                                ...
                                --%><g:if test="${result.acceptedConceptName}">&nbsp;&ndash;&nbsp;<bie:formatSciName
                                            rankId="${result.rankID}" taxonomicStatus="accepted"
                                            name="${result.acceptedConceptName}"/></g:if><%--
                                --%><g:if test="${result.favourite}"><span
                                            class="favourite favourite-${result.favourite}"
                                            title="<g:message code="favourite.${result.favourite}.detail"/>"><g:message
                                                code="favourite.${result.favourite}" encodeAs="raw"/></span></g:if>
                                ...
                                <g:elseif test="${result.has("highlight") && result.get("highlight")}">
                                    <h4>
                                        ...
                                        <a href="${result.guid}">${result.name}</a>
                                    </h4>
                                ...
                                <g:if test="${result.has("idxtype") && result.idxtype == 'TAXON'}">
                                    <ul class="summary-actions list-inline">
                                        <g:if test="${result.rankID < 7000}">
                                            <li>
                                                <g:link controller="species" action="imageSearch"
                                                        params="[id: result.guid]">
                                                    <g:message code="search.details.view.image"/> ${result.rank}
                                                </g:link>
                                            </li>
                                        </g:if>
                                        ...
                                        <g:if test="${grailsApplication.config.sightings.guidUrl}">
                                            <li>
                                                <a href="${grailsApplication.config.sightings.guidUrl} ${result.guid}">
                                                    <g:message code="search.details.record"/>
                                                </a>
                                            </li>
                                        </g:if>
                                        ...
                                        <g:if test="${result.acceptedConceptID && result.acceptedConceptID != result.guid}">
                                            <li><g:link controller="species" action="show"
                                                        params="[guid: result.guid]">
                                            <g:message code="taxonomicStatus.${result.taxonomicStatus}"
                                                       default="${result.taxonomicStatus}"/></g:link>
                                            </li></g:if>
                                    </ul>
                                </g:if>
                            </li>
...
<div id="result-template" class="row hide">
    <div class="col-sm-12">
        <ol class="search-results-list list-unstyled">
            <li class="search-result clearfix">
                <h4><g:message code="idxtype.LOCALITY"/> :
                    <a class="exploreYourAreaLink" href="">
                        <g:message code="search.sort.address"/></a>
                </h4>
            </li>
        </ol>
    </div>
